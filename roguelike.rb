...
Window.loop do
	...
	Window.draw_font(50,390,$string.to_s,font)
		...
		if Input.key_push?(K_Z)
			if $mine.x == items[0].x && $mine.y == items[0].y && items[0].drop
				...
				if attack[0] == nil
					...
					message("#{enemys[attack[1]].name}にダメージを与えられなかった")
				else
					message("#{enemys[attack[1]].name}に#{attack[0]}ダメージ与えた")
					if enemys[attack[1]].hp <= 0
						message("#{enemys[attack[1]].name}は倒れた")
